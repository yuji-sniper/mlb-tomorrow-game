...
type LineVerifyDataError = {
  error: string;
  error_description: string;
}

export async function verifyLineTokenAction(
  lineIdToken: string
  ...
  const lineVerifyResponse = await fetch('https://api.line.me/oauth2/v2.1/verify', {
    method: 'POST',
    headers: {
  ...
  const lineVerifyData = data as LineVerifyDataSuccess;

  return lineVerifyData;
}
